...
public class State {
    ...
    private final ac.ahocorasick.trie.State rootState;
     ...
     */
    private Map<Character, ac.ahocorasick.trie.State> success = new TreeMap<Character, ac.ahocorasick.trie.State>();

    /** if no matching states are found, the failure state will be returned */
    private ac.ahocorasick.trie.State failure = null;
    ...
    private ac.ahocorasick.trie.State nextState(Character character, boolean ignoreRootState) {
        ac.ahocorasick.trie.State nextState = this.success.get(character);
    ...
    public ac.ahocorasick.trie.State nextState(Character character) {
        return nextState(character, false);
    }

    public ac.ahocorasick.trie.State nextStateIgnoreRootState(Character character) {
        return nextState(character, true);
    }

    public ac.ahocorasick.trie.State addState(Character character) {
        ac.ahocorasick.trie.State nextState = nextStateIgnoreRootState(character);
        if (nextState == null) {
            nextState = new ac.ahocorasick.trie.State(this.depth+1);
    ...
    public ac.ahocorasick.trie.State failure() {
        return this.failure;
    }

    public void setFailure(ac.ahocorasick.trie.State failState) {
        this.failure = failState;
    }

    public Collection<ac.ahocorasick.trie.State> getStates() {
