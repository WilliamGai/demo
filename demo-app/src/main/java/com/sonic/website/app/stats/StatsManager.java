...
import com.sonic.website.app.websocket.WebSocket;
...
public class StatsManager extends ManagerBase {
	...
	public void init() {
		...
		try {
			...
			PortInstance.inst().addQueue(p -> {
				...
				Map<String, Long> sn_time_map_temp = dataService.getRecordList2Map(LIST_NAME,
				            x -> x.getTime() > time_today_begin);
				sn_time_map_all.putAll(sn_time_map_all_temp);
				sn_time_map.putAll(sn_time_map_temp);
				LogCore.BASE.info(
				            "StatsManager init, STATS_TIME_TG={},sn_time_map.size={},sn_time_map_all.size={}",
				            date_tag, Util.size(sn_time_map_temp), Util.size(sn_time_map_all_temp));
			});
		} catch (Exception e) {
	 ...
	 * @param key,主键set名称
	 * @param name
	 * @param value TODO,zset记录次数，hset记录时间
	 */
	...
	public List<DataVO> getBaseInfos() {
		...
		base_info_list
		            .add(new DataVO("LRU cache hit rate", DataManager.inst().userBiInfoCache.getStats().hitRate()));
	...
	public List<DataVO> getUriStatsAllTime() {
		...
		list.add(new DataVO("总计", a_time.get() / NAOS_1000000, df.format(_avtime_all / NAOS_1000000),
		            a_count.get()));
		return list;
	}
	...
	public List<DataVO> getUriStatsThisTime() {
		...
		double _avtime_all = (double) a_time.get() / a_count.get();
		uri_list.add(new DataVO("总计", a_time.get() / NAOS_1000000, df.format(_avtime_all / NAOS_1000000),
		            a_count.get()));
		return uri_list;
	}
